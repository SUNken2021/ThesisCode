...
class TransDecoder(nn.Module):
    def __init__(self, args) -> None:
        ...
        decoder_layer = nn.TransformerDecoderLayer(
            d_model=args.kspace_size, nhead=5, batch_first=True)
    ...
    def count_loss(self, dataset):
        ...
        loss = loss_fc(ground_truth[:, 2:, :], output[:, 2:, :])/(
            batch_size * (tlength - 2)
        )
        return loss
    ...
    def test_loss_short(self, dataset):
        ...
        loss = self.loss_fc(output[:, -1:, :], predict)
        return loss
    ...
    def test_loss_long(self, dataset):
        ...
        for i in range(N):
            decoder_input = torch.cat(
                (given, prediction[:, :i, :]), dim=1
            )
            output = self.forward(given, decoder_input)
            prediction[:, i, :] = output[:, -1, :]
